...
from dagster._core.definitions.freshness_checks.non_partitioned import (
    build_non_partitioned_freshness_checks,
)
...
def test_params() -> None:
    ...
    with pytest.raises(CheckError, match="At least one of freshness_cron or maximum_lag_minutes"):
        result = build_non_partitioned_freshness_checks(assets=[my_asset])

    result = build_non_partitioned_freshness_checks(assets=[my_asset], maximum_lag_minutes=10)
    assert len(result) == 1
    assert next(iter(result[0].check_keys)).asset_key == my_asset.key

    result = build_non_partitioned_freshness_checks(assets=[my_asset.key], maximum_lag_minutes=10)
    assert len(result) == 1
    assert next(iter(result[0].check_keys)).asset_key == my_asset.key

    src_asset = SourceAsset("source_asset")
    result = build_non_partitioned_freshness_checks(assets=[src_asset], maximum_lag_minutes=10)
    assert len(result) == 1
    assert next(iter(result[0].check_keys)).asset_key == src_asset.key

    result = build_non_partitioned_freshness_checks(
    ...
    with pytest.raises(Exception, match="Found duplicate assets"):
        build_non_partitioned_freshness_checks(assets=[my_asset, my_asset], maximum_lag_minutes=10)
...
def test_different_event_types(
    ...
    with pendulum.test(start_time):
        freshness_checks = build_non_partitioned_freshness_checks(
            assets=[my_asset],
...
# Test for providing partitioned assets to build_non_partitioned_freshness_checks
